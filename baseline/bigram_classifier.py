...
def extractBigramFeatures(x):
	...
	lastWord = None
	for word in x.split():
		if (lastWord is None): continue
		retval[(lastWord, word)] = retval[(lastWord, word)] + 1.0
	return retval
...
def getReadmes(num):
	...
	'''
	try:
		conn=psycopg2.connect("dbname='data-collector_development' user='friedemann'")
	except:
...
def main(argv=None):
	'''
	Runs a naive bigram classifier
	'''
	if argv is None:
		argv = sys.argv
	...
	if len(argv) >= 3:
		n_training_samples = argv[1]
		n_testing_samples	= argv[2]
	else:
	...
	# featureExtractor = extractBigramFeatures
